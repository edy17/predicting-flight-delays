...
with DAG("flight_delays_modeling_dag",
    ...
    local_data_location = f"{os.environ['AIRFLOW_HOME']}/data/task"
    raw_data_location = f"file://{local_data_location}"
    lakehouse_location = f"file://{os.environ['AIRFLOW_HOME']}/data/spark-warehouse"
    ...
    def flight_data_ingestion(params):
        ...
        year = params["year"]
        save_path = f"{local_data_location}/{year}/flights"
        ...
        for month in processed_months:
            ...
            with ZipFile(buffer) as zip_file:
                files = [zip_file.extract(file, save_path) for file in zip_file.namelist() if file.endswith(".csv")]
            if total_size != 0 and not os.path.isfile(files[0]):
                raise Exception(f"Could not download zip file : {url}")
            print(f"{files[0]} file download done")
        print("::endgroup::")
    ...
    def weather_data_ingestion(params):
        ...
        os.makedirs(f"{local_data_location}/{year}/weather", exist_ok=True)
        response = requests.get(url, stream=True, allow_redirects=True)
        ...
        for file_name in known_stations:
            ...
            filepath = f"{local_data_location}/{year}/weather/{file_name}"
            ...
            with open(filepath, "wb") as file:
                for data in response.iter_content(chunk_size):
                    file.write(data)
            if total_size != 0 and not os.path.isfile(filepath):
                raise RuntimeError(f"Could not download file : {filepath}")
            print(f"{file_name} file download done")
        print("::endgroup::")
